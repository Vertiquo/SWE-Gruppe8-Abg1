...
-- (4) chown postgres:postgres /var/lib/postgresql/tablespace/monitor
-- (5) exit
-- (6) docker compose down
-- (7) in docker-compose.yaml den User "postgres" wieder aktivieren
-- (8) docker compose up
-- (9) docker compose exec postgres bash
-- (10) psql --dbname=postgres --username=postgres --file=/sql/create-db-monitor.sql
-- (11) exit

-- https://www.postgresql.org/docs/current/sql-createrole.html
CREATE ROLE monitor LOGIN PASSWORD 'p';

-- https://www.postgresql.org/docs/current/sql-createdatabase.html
CREATE DATABASE monitor;

GRANT ALL ON DATABASE monitor TO monitor;

-- https://www.postgresql.org/docs/10/sql-createtablespace.html
CREATE TABLESPACE monitorspace OWNER monitor LOCATION '/var/lib/postgresql/tablespace/monitor';
